...
- La classe Query è stata oggetto di attenta revisione che ha comportato l'aggiunta di alcuni comportamenti e la rimozione di altri non reputati necessari nell'ambito del corrente progetto.
  ...
  - Introduzione del supporto per operazioni JOIN SQL con eager loading gerarchico multi-entità, assente nella libreria originale. È stato implementato un nuovo enum `JoinType` per gestire i diversi tipi di JOIN (INNER, LEFT, RIGHT, CROSS) in modo type-safe. Le classi `BaseAdapter`, `Query`, `BaseModel` e `BaseResultSet` sono state estese per supportare sia JOIN espliciti tramite foreign key che relazioni nested a più livelli utilizzando dot notation (`'author.country.continent'`) e sintassi ad array nested (`['author' => ['country' => ['continent']]]`). L'hydration gerarchica è completamente automatica e integrata con il sistema di cache delle entità esistente per evitare duplicazioni. Il sistema supporta nativamente anche le relazioni one-to-many inverse tramite batch loading ottimizzato e le relazioni ricorsive tramite `SelfReferencedEntity`. L'implementazione è completamente trasparente e non introduce breaking changes, rilevando automaticamente la presenza di JOIN nei metadati della query.
